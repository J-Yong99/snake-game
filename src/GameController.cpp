...
void GameController::game_control(int arr[][60],int& i){
    switch (i) {
    case 0:
        snakeColor=4;
        speed=100000;
...
void GameController::game_run(int m4[][60],int& i){
...
void GameController::check_snake_eat(){
...
void GameController::item_setter(){
...
void GameController::render_map(){
        clear_map();
        set_snake();
...
int GameController::keyboard_input(){
    int input = getch();
    switch(input)
    {
        case UP:
...
void GameController::set_item(Item item){
    map.m[item.x][item.y] = item.itemType ? GITEM : PITEM;
}

void GameController::expired_item(Item item){
    map.m[item.x][item.y] = EMPTY_SPACE;
}

void GameController::check_item(vector<Item> &v){
...
bool GameController::is_empty(int x, int y){
